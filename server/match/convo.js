...
async function startConvo(agentAId, agentBId) {
  ...
  const existing = await db.query('SELECT * FROM mx_conversations WHERE agent_a=$1 AND agent_b=$2', [a, b]);
...
async function postMessage(convoId, senderAgentId, content, msgType, scenarioKey, metadata) {
  ...
  const cv = await db.query('SELECT * FROM mx_conversations WHERE id=$1', [convoId]);
...
async function requestContact(convoId, initiatorAgentId) {
  ...
  await db.query("UPDATE mx_conversations SET status='contact_requested' WHERE id=$1", [convoId]);
  return { ok: true };
}
...
async function approveContact(convoId, userId, myContact) {
  ...
  await db.query(`UPDATE mx_contact_requests SET ${col} WHERE convo_id=$2`, [myContact, convoId]);

  const cr = await db.query('SELECT * FROM mx_contact_requests WHERE convo_id=$1', [convoId]);
  const r = cr.rows[0];
  if (r.user_a_approved && r.user_b_approved) {
    await db.query("UPDATE mx_contact_requests SET status='completed' WHERE convo_id=$1", [convoId]);
    await db.query("UPDATE mx_conversations SET status='contact_shared' WHERE id=$1", [convoId]);
    return {
      completed: true,
